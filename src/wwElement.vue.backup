<template>
  <div 
    ...
    :style="{ 
      '--toast-width': content.width || '356px',
      '--toast-gap': content.gap || '8px'
    }"
  >
    <TransitionGroup
      :name="transitionName"
    ...
    >
      ...
      >
        <!-- Swipe indicator -->
        <div
          v-if="content.enableSwipe && toast.swipeProgress > 0"
        ...
        <!-- Close button -->
        <button
          v-if="content.showCloseButton && !toast.action && !toast.cancel"
        ...
        <!-- Progress bar -->
        <div
          v-if="toast.duration && content.showProgress"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const toasts = ref([])
    const maxToasts = computed(() => props.content.maxToasts || 5)
    ...
    const getToastTransform = (toast) => {
      ...
      if (props.content.position?.includes('bottom')) {
    ...
    const handleToastClick = (toast) => {
      if (props.content.dismissOnClick !== false) {
        dismissToast(toast.id)
      }
    }
    ...
    onMounted(() => {
      ...
      if (props.content.toasts) {
        props.content.toasts.forEach(toastData => {
    ...
    watch(() => props.content.toasts, (newToasts) => {
